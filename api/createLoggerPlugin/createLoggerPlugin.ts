      ...
      return {
        async executionDidStart({ operationName, operation }) {
          ...
          loggerWithSession.info(
            `apollo:execution did start --- ${typeName} ${operationName}...`,
          )
        },
        ...
        async didResolveOperation({
          ...
          loggerWithSession.info(
            { kind, name, params },
            `apollo:didResolveOperation > > > > > > > > > ${kind} ${name}`,
          )
        },

        async parsingDidStart(_requestContext) {
        ...
        async validationDidStart() {
          ...
          return async (err?: ReadonlyArray<Error>) => {
            if (err && err.length > 0) {
              ...
              loggerWithSession.info(
                'apollo:validation did end. Document cached.',
              )
            }
          }
        },
        ...
        async willSendResponse({ operation, operationName }) {
          ...
          loggerWithSession.info(
            { kind, name, duration },
            `apollo:will send response < < < < < < < < < ${kind} ${name} in ${duration}ms`,
          )
        },
