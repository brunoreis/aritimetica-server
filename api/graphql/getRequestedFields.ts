...
const getRequestedFieldsRecursive = (selections: readonly any[], parentFieldName = ''): string[] => {
  ...
  for (const selection of selections) {
    if (selection.kind === 'Field') {
      ...
      if (selection.selectionSet) {
        requestedFields.push(...getRequestedFieldsRecursive(selection.selectionSet.selections, fieldName));
      }
...
const getRequestedFields = (resolverInfo:GraphQLResolveInfo):string[] => {
  ...
  if(resolverInfo.fieldNodes[0].selectionSet) {
    requestedFields = getRequestedFieldsRecursive(resolverInfo.fieldNodes[0].selectionSet.selections)
  }
  return requestedFields
}
