...
type IncludeFields = {
  ...
  assignedLessons?:
    | boolean
    | { include: { assignee: boolean; assigner: boolean } }
...
const getIncludeFields = (requestedFields: string[]): IncludeFields => {
  ...
  if (requestedFields.includes('assignedLessons')) {
    ...
    if (
      requestedFields.includes('assignedLessons.assignee') ||
      requestedFields.includes('assignedLessons.assigner')
    ) {
      ...
      authorize: async (_root, args, ctx: ContextType) => {
        return (
          ...
          ((await ctx.auth.hasGlobalPermission(
            ...
            (await ctx.auth.isCurrentUser(args.userUuid))) ||
          ((await ctx.auth.hasGlobalPermission(
            permissions.view_users_of_my_groups.uuid,
          )) &&
            (await ctx.auth.shareGroupWithCurrentUser(args.userUuid)))
        )
      },
      resolve: async (
        _root,
